...
struct windowSetup *initScreenFromElement( TiXmlElement *screen )
{
  ...
  //if (window==wmain)
  {
    ...
    /**
    buttonAttr.x = menuAttr.w-70; // about Mika button
    buttonAttr.y = 5;
    buttonAttr.w = 60;
    buttonAttr.h = 60;
    button = tr_ButtonInitEx2(NULL,&buttonAttr,cmain);
    ...
    tr_CanvasAddObjStatic(ctitlecanvas,button);
    tr_SetAction(button,onMika,TR_ACTION_BUTTON);
    //
    image = tr_ImageInitEx(NULL,&buttonAttr,(char*)"img/mikaButton.jpg");
    tr_ImageSetRound(image,2);
    tr_AddImage(button,image);
    tr_ImagePosCenter(image,button);
    */
  ...
  if (window!=wmain)
  {
    ...
    image = tr_ImageInitEx(NULL,&buttonAttr,(char*)"img/backarrow.jpg");
    tr_AddImage(button,image);
    tr_ImagePosCenter(image,button);
  }
...
int setupItemScreen( struct windowSetup *ws )
{
  ...
  if (ordermenu)
  {
    ...
    struct gui_Widget *w = button->widget.sub;
    tr_ChangeTextExPos(button,text,w->attr.w-75,25); // 250,25
...
struct windowSetup *generateScreens( TiXmlElement *root, windowSetup *rootws )
{
  ...
  else
  {
    ...
    for(n = 0; n < itemCount; n++)
    {
      ...
      buttonAttr.x = 0;
      buttonAttr.y = 0;
      buttonAttr.w = mainAttr.w-80; // 375
      buttonAttr.h = 70;
...
int main( )
{
  ...
  menuAttr.h = mainAttr.h-menuAttr.y;;
  ...
  #ifndef _USE_SKIN
    tr_SetColor(cordermenu,(char*)"#101010");
  #endif
  ...
  tr_SetColor(button,(char*)"#000000");
  tr_CanvasAddObjStatic(cmika,button);
  image = tr_ImageInitEx(NULL,&buttonAttr,(char*)"img/mikaPoster.jpg");
  tr_AddImage(button,image);
  tr_ImagePosCenter(image,button);
  ...
  #ifndef _USE_SKIN
    tr_SetColor(cmainmenu,(char*)"#050505");
  #endif
