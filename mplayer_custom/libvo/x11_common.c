...
//       printf("\rEvent.type=%X  \n",Event.type);
        switch (Event.type)
        {
            ...
            case ButtonRelease:
                ...
                // mika - extremely dirty fast hack
              
                fd = fopen("mikaSignageWindow.log","r");
                if (fd)
                {
                  fgets((char*)data,255,fd);
                  fclose(fd);
                  if (data[strlen((char*)data)-1]=='\n')
                  data[strlen((char*)data)-1] = 0x00;
                  int wid = atoi((char*)data);
                
                  XEvent event;
                  event.xbutton.window = wid;
                  event.xbutton.display = mydisplay;
                                   ...
                                   int depth, Colormap col_map)
{
    ...
    // mika - extremely dirty fast hack
    unsigned char data[256];
    FILE *fd = fopen("mikaSignageWindow.log","r");
    if (fd)
    {
      // get the ID of the Mika parent window and create this window
      // as a subwindow
      //
      fgets((char*)data,255,fd);
      fclose(fd);
      ...
      ret_win =
        XCreateWindow(mDisplay, wid, x, y, width, height, 0, depth,
                      CopyFromParent, vis, xswamask, &xswa);
      if (!ret_win)
      {
        printf("mplayer x11_common couldn't create Mika subwindow\n");
        exit(2);
      }
    }
    else
