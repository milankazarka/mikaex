...
// mika
#define _VO_SILENT_NO
#define _BRIDGE_FILENAME "/work/mikaex/mikaAccessBackground.log"
static void Display_Image(XImage * myximage, uint8_t * ImageData)
{
    
    static int bridgeSEM = 0;
    static int bridgeSHM = 0;
    static int nx;
    static int ny;
      ...
      )
    {
      ...
      FILE *fd = fopen(_BRIDGE_FILENAME,"r");
      if (fd)
      {
        fgets((char*)line,64,fd);
        if (line[strlen((char*)line)-1]=='\n')
          line[strlen((char*)line)-1]=0x00;
        bridgeSEM = atoi((char*)line);
        
        fgets((char*)line,64,fd);
        if (line[strlen((char*)line)-1]=='\n')	
          line[strlen((char*)line)-1]=0x00;
        bridgeSHM = atoi((char*)line);
        
        fclose(fd);
      }
      else
      {
          printf("Display_Image couldn't open bridge (%s)\n",
              _BRIDGE_FILENAME);
      }
    }
    
    printf("Display_Image size(%dx%d) sem(%d) shm(%d)\n",
        dst_width,myximage->height,
        bridgeSEM,bridgeSHM);    
            
    binarySemaphoreWait(bridgeSEM);
    void *dest = shmat(bridgeSHM,0,0);
        
    for(ny = 0; ny < myximage->height; ny++)
    {
        for(nx = 0; nx < dst_width; nx++)
        {
            memcpy(dest+(dst_width*ny*4)+(nx*4),myximage->data+(dst_width*ny*4)+(nx*4),4);
            memset(dest+(dst_width*4*ny)+(nx*4)+3,0xff,1);
        }
    }
        
    shmdt(dest);  
    binarySemaphorePost(bridgeSEM);
