...
#define grAlloc  malloc
                               ...
                               int            h )
  {
    ...
    if ( !gr_x11_blitter_reset( &blit, &surface->root.bitmap,
                                ...
                                x, y, w, h ) )
    {
      if (surface->ximage_dest!=NULL)
      {
      //printf("gr_x11_surface_refresh_rect\n");
      ...
      for (yn = y; yn < y+h; yn++)
      {
        for (xn = x; xn < x+w; xn++)
        {
          pixel = XGetPixel(surface->ximage,xn,yn);
          XPutPixel(surface->ximage_dest,xn,yn,pixel);
        }
      }
      }
    }
  ...
  static long 
  gr_x11_surface_get_sys_id( grX11Surface *surface )
  {
    return surface->win;
  }

  static void
  gr_x11_surface_set_sys_id( grX11Surface *surface, void *win )
  {
    surface->win = win;
  }

  static void
  gr_x11_surface_set_pixbuf( grX11Surface *surface, void *pixbuf )
  {
    //printf("gr_x11_surface_set_pixbuf\n");
    surface->ximage_dest = pixbuf;
  }

  static int
  gr_x11_surface_init( Window	      *win,
                       grX11Surface*  surface,
                       grBitmap*      bitmap )
  {
    Display*            display;
    int                 screen;
    ...
    surface->root.set_pixbuf   = (grSetPixbufFunc)  gr_x11_surface_set_pixbuf;
    ...
    surface->root.get_sys_id   = (grGetSysIdFunc)   gr_x11_surface_get_sys_id;
    surface->root.set_sys_id   = (grSetSysWindowFunc) gr_x11_surface_set_sys_id;
