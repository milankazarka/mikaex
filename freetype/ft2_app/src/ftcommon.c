...
#define DIM_Y      480
  ...
  FTDemo_Display* _ftDemo_display_new( grPixelMode  mode, void *win, int w, int h )
  {
    ...
    surface = grNewSurface( win, 0, &bit );
    ...
    grSetGlyphGamma( 1.0 ); // 1.0

    memset( &display->fore_color, 0x00, sizeof( grColor ) ); // 0
    memset( &display->back_color, 0xff, sizeof( grColor ) );

    return display;
  }

  FTDemo_Display* FTDemo_Display_New( grPixelMode  mode, void *win )
  {
    return _ftDemo_display_new( mode, win, DIM_X, DIM_Y );
  }

  FTDemo_Display* FTDemo_Display_NewEx( grPixelMode  mode, void *win, int w, int h )
  {
    return _ftDemo_display_new( mode, win, w, h );
  }
...
#define TRUNC( x )  (   (x) >> 6 )
                   ...
                   FT_Size*        asize )
  {
    ...
    //printf("__ft FTDemo_Get_Size\n");

    error = FTC_Manager_LookupSize( handle->cache_manager, &handle->scaler, &size );

    __FT_Size *_size = (struct __FT_Size*)size;
    //printf("__ft FTDemo_Get_Size FT_Size(%d/%d)\n",
    //  _size->metrics.x_ppem,
    //  _size->metrics.y_ppem);
  ...
  string_load( FTDemo_Handle*  handle )
  {
    ...
    for ( n = 0; n < handle->string_length; n++ )
    {
           ...
           !FT_Get_Glyph( face->glyph, &glyph->image ) )
      {
        ...
        /*printf("__ft string_load (%d/%d-%d/%d-%d/%d)\n",
          glyph->vvector.x,
          glyph->vvector.y,
          glyph->vadvance.x,
          glyph->vadvance.y,
                         ...
                         FT_Vector*              advances )
  {
    ...
    //printf("__ft string_render_prepare\n");
    ...
    for ( i = 0; i < handle->string_length; i++ )
    {
      ...
      int _len = ((float)advances[i].x/(float)face->units_per_EM)*(float)18.0;
    
      /*printf("__ft advance (%d) extent.x(%d) units/EM(%d) x_scale(%d) len(%d)\n",
        advances[i].x,
        extent.x,
        face->units_per_EM,
    ...
    /*for (i = 0; i < handle->string_length; i++)
    {
      printf("__ft advance2 (%d) units/EM(%d) x_scale(%d)\n",
        advances[i].x,
        face->units_per_EM,
        face->size->metrics.x_scale);
    }*/
                      ...
                      int                     y )
  {
    ...
    for ( n = 0; n < handle->string_length; n++ )
    {
      ...
      //printf("FT size (%d %d %d %d)\n",bbox.xMin,bbox.xMax,bbox.yMin,bbox.yMax); // mika

#if 1
      //if ( n == 0 )
      {
        //printf( "__ft bbox = [%ld %ld %ld %ld]\n",
        //    bbox.xMin, bbox.yMin, bbox.xMax, bbox.yMax );
      }
#endif

      if ( bbox.xMin>handle->width ) handle->width += bbox.xMin-handle->width;
      handle->width 	+= ( bbox.xMax - bbox.xMin );	// mika - catch the size of the drawn
      if ( bbox.yMax - bbox.yMin > handle->height )
        handle->height 	= ( bbox.yMax - bbox.yMin );	// string
