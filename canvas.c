...
#include "mikaSkin.h"
  ...
  */
  ...
  void *_expose(void*data)
  {
    gui_TriggerExpose(NULL);
  }
  ...
  int form_CanvasMotion( struct gui_Event *event )
  {
    ...
    #ifdef _DEBUG
      printf("form_CanvasMotion can(%p)\n",can);
    #endif
    ...
    if (lastx!=-1&&lasty!=-1)
    {
      if (DEBUG==TRUE) printf("form_CanvasMotion (%d)\n",event->y-lasty);
  ...
  /**  button press when applied _AI logic
   */
  ...
  */
  ...
  /** expose the canvas and all it's children
   *
   */
  int form_CanvasExpose( struct gui_Widget *w )
  {
    ...
    // levels represent the level in which we are in
    //
    for (n = GUI_LEVEL_MIN; n < GUI_LEVELS; n++)
    {
     ...
     while(cano!=NULL)
     {
      ...
      if (cano->widget!=NULL)
      {
        if (cano->widget->attr.bvisible==FALSE)
        {
          cano = cano->next;
          continue;
        }
        ...
        {
          if (cano->widget->attr.onexposeEx)
          {
            cano->widget->attr.onexposeEx(
              cano->widget,
              cano->pos.x,
              cano->pos.y,
              cano->pos.w,
  ...
  /** move all canvas objects which aren't static
   */
  int form_CanvasMove( struct gui_Widget *w, int val )
  {
    ...
    #ifdef _DEBUG
      printf("form_CanvasMove move_by (%d)\n",val);
    #endif
    ...
    #ifdef _DEBUG
      printf("form_CanvasMove yhigh(%d)\n",yhigh);
    #endif
    ...
    #ifdef _DEBUG
      printf("form_CanvasMove move_by 2 movey(%d) val(%d)\n",can->movey,val);
    #endif
    can->movey += val;
    return 0;
  }
  ...
  */
  ...
  /** exchange an object with another object
   *
   */
  int form_CanvasExchangeObjectStatic( struct gui_Widget *w, struct gui_Widget *original, struct gui_Widget *child )
  {
    #ifdef _DEBUG
      printf("form_CanvasExchangeObjectStatic\n");
    #endif
    if (!w || !original || !child)
    {
      #ifdef _DEBUG
        printf("form_CanvasExchangeObjectStatic error, invalid arguments\n");
      #endif
      return 1;
    }
    
    static struct form_Canvas *can = NULL;
    struct _can_obj *current = can->can_pool;
    
    while(current)
    {
      if (current->widget==original && current->type==CAN_OBJ)
      {
        #ifdef _DEBUG
          printf("form_CanvasExchangeObjectStatic hit, echange widget\n");
        #endif
        current->widget = child;
        break;
      }
  ...
  int form_CanvasSetMov( struct gui_Widget *w, int movetype )
  {
    ...
    if (movetype==CAN_MOV_SCROLL)
    {
      ...
      node = mikaSkinGlobalGetNode((unsigned char*)"mikaCanvasButtons");
      
      if (node)		gui_WidgetSetColor(can->wup,(char*)node->color.hex);
      else		gui_WidgetSetColor(can->wup,"#202020"); // "trans");
      ...
      if (img_aup) gui_WidgetAddImage(can->wup,img_aup);
        ...
        strcpy((char*)image->filename,CANVAS_UP);
        gui_CreateImage(gui_GetMain(),image);
        gui_WidgetAddImage(can->wup,image);
      #else
      ...
      #ifdef CANVAS_EX
        can->wup->attr.x = (w->attr.w-DEFA_CSMARRL)+30;
        can->wup->attr.y = (w->attr.h/2)-(DEFA_CSMARRL+10)+40;
        can->wup->attr.w = DEFA_CSMARRL-50;
        can->wup->attr.h = DEFA_CSMARRL-50;
      #else
        can->wup->attr.x = (w->attr.w-DEFA_CSMARRL)+50;
        can->wup->attr.y = (w->attr.h/2)-(DEFA_CSMARRL+10)+40;
        can->wup->attr.w = DEFA_CSMARRL-60;
        can->wup->attr.h = DEFA_CSMARRL-60;
      #endif
      ...
      if (node)		gui_WidgetSetColor(can->wdown,(char*)node->color.hex); // "trans");
      else		gui_WidgetSetColor(can->wdown,"#202020"); // "trans");
      
      gui_WidgetSetRound(can->wdown,2);
      if (img_adown) gui_WidgetAddImage(can->wdown,img_adown);
      
      #ifdef CANVAS_EX
        struct gui_Image *imageDOWN = (struct gui_Image*)malloc(sizeof(struct gui_Image));
        strcpy((char*)imageDOWN->filename,CANVAS_DOWN);
        gui_CreateImage(gui_GetMain(),imageDOWN);
        gui_WidgetAddImage(can->wdown,imageDOWN);
      #else
      ...
      #ifdef CANVAS_EX
        can->wdown->attr.x = (w->attr.w-DEFA_CSMARRL)+30;
        can->wdown->attr.y = (w->attr.h/2)+10;
        can->wdown->attr.w = DEFA_CSMARRL-50;
        can->wdown->attr.h = DEFA_CSMARRL-50;
      #else
        can->wdown->attr.x = (w->attr.w-DEFA_CSMARRL)+50;
        can->wdown->attr.y = (w->attr.h/2)+10;
        can->wdown->attr.w = DEFA_CSMARRL-60;
        can->wdown->attr.h = DEFA_CSMARRL-60;
      #endif
