...
#define GOID_TEXTSIMPOS	300	// text at an absolute position
#define GOID_TROP	3
#define GOID_COLOR	4	// color from
#define GOID_COLOR2	41	// color to
...
static struct _goid_id_descr _goid_id_descr_pool[15] = {
    ...
    { GOID_WIDGET,	0,	(char*)"goid_widget" },		// subwidget
...
#define FORM_SIMPLE	99	// simple form with general data and cfg
  ...
  #define FORM_LISTBOX	16	// listbox
  #define FORM_LISTBOXITEM 17	// listbox item
  #define FORM_CUSTOM	999	// custom item
  ...
  #define CANVAS_UP	"img/bup.jpg"
  #define CANVAS_DOWN	"img/bdown.jpg"
  #define CANVAS_EX_NO
...
typedef struct gui_WidgetAttributes
{
    ...
    BOOL		bvisible;	// is the widget visible
    BOOL		bvirtual;
    int			w;
    int			h;
    int			x;
...
typedef struct gui_Bounds
{
  ...
  typedef struct form_CanvasDelegate
  {
    void		*obj;
    void		*(*onDown)(void*obj);
    void		*(*onUp)(void*obj);
  } form_CanvasDelegate;
  ...
  typedef struct form_Canvas
  {
    ...
    int			movestep;	// step by which to move the canvas
    ...
    struct form_CanvasDelegate	*delegate;
  } form_Canvas;
  
  typedef struct gui_Scene
  {
    int 		id;
    struct trop_List	*list;
  } gui_Scene;
  
  typedef struct gui_SceneEx
  {
  ...
  typedef struct _gui_layout_link
  {
    char		from[64];
    char		to[64];
  } _gui_layout_link;
  
  typedef struct _gui_layout_widget
  {
    char		name[64];
  } _gui_layout_widget;
  
  typedef struct gui_Layout			// inner representation of gui layout (gui scripting)
  {
    char		name[64];
  ...
  */
  ...
  typedef struct _rec_expose_rect
  {
    int				x;
    int				y;
    int				w;
  ...
  typedef struct _rec_expose_txt
  {
    int				x;
    int				y;
    int				w;
  ...
  typedef struct _rec_expose_bit
  {
    int				type;
    void			*sub;
  ...
  typedef struct form_ListboxItem
  {
    struct gui_Widget 		*holder;
    struct form_ListboxItem 	*next;
  } form_ListboxItem;
  
  /** attributes of a listbox item
   */
  typedef struct listboxItemSetup
  {
    unsigned char str_big[256];
    unsigned char str_small[256];
    unsigned char str_right[256];
    unsigned char color_bg[12];
  } listboxItemSetup;
  
  typedef struct form_ListboxDelegate
  {
    // number of all scrollable items
    void			*(*numberOfItems)(int *nitems);
    struct listboxItemSetup	*(*listboxItemSetup)(int nitem); // item setup
  } form_ListboxDelegate;
  
  /** a simpler way of handling alist of items, better
   *  than just loading them into the Canvas
   */ 
  typedef struct form_Listbox
  {
    struct form_Canvas		*canvas;
    struct form_ListboxDelegate *delegate;
    struct form_ListboxItem 	*items; // items on screen
    int				move; 	// moving up and down between items
    int				first;
    int				last;
  } form_Listbox;
  ...
  /** extended slidebar
   */
  typedef struct form_SlidebarEx
  {
    int 			total;		// total sum
    int				position;	// position in sum
    int				size;		// size in total sum
    struct gui_Widget		*delegate;	// relay object
  } form_SlidebarEx;
...
int gui_WidgetResetColor( struct gui_Widget *w );
...
int form_CanvasSetMovestep( struct gui_Widget *w, int movestep );
...
/** exchanging static objects is a workarround that enables us changing the content of the screen
 *  without changing the window - it can also be achieved by changing the canvas itself, but that's not needed
 */
int form_CanvasExchangeObjectStatic( struct gui_Widget *can, struct gui_Widget *original, struct gui_Widget *child );
...
int form_ListboxSetTextColor( struct gui_Widget *widget, unsigned char w, unsigned char g, unsigned char b, unsigned char a );
