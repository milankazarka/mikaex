...
#define _UDS_EVENTS
  ...
  int refreshGuiLow( struct platform_low_t *platform_low )
  {
    ...
    #ifdef _DEBUG
      if (DEBUG==TRUE) printf("__interface refreshGuiLow dpy(%d) screen(%d)\n",
        platform_low->dpy,
        platform_low->screen);
    #endif
    ...
    #ifdef _DEBUG
      if (DEBUG==TRUE) printf("__interface refreshGuiLow w(%d) h(%d)\n",
        rw,rh);
    #endif
    ...
    #else    
      #ifdef _UDS_EVENTS
        activityEventExposeOverSocket(&bigInterface->activity);
      #endif
    #endif
  ...
  int initGuiLow( struct platform_low_t *platform_low )
  {
    ...
    else
    {
    ...
    #ifdef _DEBUG
      if (DEBUG==TRUE) printf("XCreateWindow pre dpy(%d) screen(%d) geom(%dx%d)\n",
      ...
      #ifdef _DEBUG
        if (DEBUG==TRUE) printf("XCreateWindow after\n");
      #endif
      ...
      {
        #ifdef _DEBUG
          if (DEBUG==TRUE) printf(":Xlib wid %p\n",platform_low->id);
        #endif
      }
    ...
    for(format = formats; count > 0; count--, format++)
    {
      if (format->depth == DefaultDepth(platform_low->dpy,platform_low->screen))
      {
        ...
        #ifdef _DEBUG
          if (DEBUG==TRUE) printf("__interface platform_low->surface creating (%dx%d)\n",rw,rh);
        #endif
  ...
  { 
    ...
    #ifdef _DEBUG
      if (DEBUG==TRUE) printf("__interface guiFlushLow\n");
    #endif
    ...
    else
    {
      ...
      else
      {
          ...
          { 
            ...
            {
              #ifdef _DEBUG
                if (DEBUG==TRUE) printf("nextGuiEventLow activityEventActive = 1\n");
              #endif
              ...
              #endif
              binarySemaphorePost(bigInterface->controller.SEMID); // breaks the while loop
              // todo - remove X emulation
              break;
            }
  ...
  int triggerExposeLow( struct platform_low_t *platform_low )
  {
    if (!platform_low) return -1;
      ...
      #ifdef _DEBUG
        if (DEBUG==TRUE) printf("triggerExposeLow\n");
      #endif
    #endif
      ...
      while(1)
      {
        #ifdef _TRIGGER_EXPOSE_DEBUG
          #ifdef _DEBUG
            if (DEBUG==TRUE) printf("triggerExposeLow poll\n");
          #endif
        #endif
        //binarySemaphoreWait(bigInterface->controller.SEMID);
        #ifdef _TRIGGER_EXPOSE_DEBUG
          #ifdef _DEBUG
            if (DEBUG==TRUE) printf("triggerExposeLow poll synced\n");
          #endif
        #endif
        #ifdef _TRIGGER_EXPOSE_CHECKS
          if (activityEventActive(&bigInterface->activity)==0)
        #endif
        {
        ...
        #ifdef _TRIGGER_EXPOSE_DEBUG
          #ifdef _DEBUG
            if (DEBUG==TRUE) printf("triggerExposeLow poll cycle\n");
          #endif
        #endif
        usleep(_CLOCKTIK);
      }
      #ifdef _TRIGGER_EXPOSE_DEBUG
        #ifdef _DEBUG
          if (DEBUG==TRUE) printf("triggerExposeLow poll finish\n");
        #endif
      #endif
    #else
      ...
      event.xexpose.x = 0;
      event.xexpose.y = 0;
      event.xexpose.width = 640;
      event.xexpose.height = 480;
    
      int out = XSendEvent(
              platform_low->dpy,
              platform_low->id, FALSE,
              ExposureMask,&event
              );
    
      if (out == BadValue || out == BadWindow)
        return -1;
    #endif
...
)
{
  ...
  #ifdef _DEBUG
    if (DEBUG==TRUE) printf("platformRefreshCommitToRoot\n");
  #endif
  ...
  void *rootSurface = shmat(bigInterface->controller.SHMID,0,0);
  if (rootSurface==-1)
  {
    #ifdef _DEBUG
      if (DEBUG==TRUE) printf("platformRefresh error could not access root surface\n");
    #endif
    exit(2);
  }
  
  static double _angle = 4.0;
  
  binarySemaphoreWait(bigInterface->activity.sem->id);
  void *shmpbuffer = shmat(bigInterface->activity.shm->id,0,0);
  memcpy(
    shmpbuffer,bigInterface->surfaceData,
    rw*rh*4
    );
  
  memset(
    rootSurface,0x00,
    _ROOT_DEFAULT_WIDTH*_ROOT_DEFAULT_HEIGHT*4);
  cairo_surface_t *destination = cairo_image_surface_create_for_data(
    rootSurface,
    CAIRO_FORMAT_ARGB32,
    _ROOT_DEFAULT_WIDTH,_ROOT_DEFAULT_HEIGHT,
    4*_ROOT_DEFAULT_WIDTH
  );
  cairo_t *_cairo = cairo_create(destination); // cairo canvas on destination
  cairo_surface_t *image = cairo_image_surface_create_for_data(
    bigInterface->surfaceData,
    CAIRO_FORMAT_ARGB32,
    rw,rh,
  ...
  cairo_rotate(_cairo,_angle*3.14/180);
  cairo_set_source_surface(_cairo,image,0,0);
  cairo_paint(_cairo);
  cairo_surface_destroy(image);
  ...
  // tell the rootController to update the surface
  #endif
  
  #ifdef _DEBUG
    if (DEBUG==TRUE) printf("platformRefreshCommitToRoot finish\n");
  #endif
...
)
{
  ...
  #ifdef _DEBUG
    if (DEBUG==TRUE) printf("platformRefreshCommitToSHM\n");
  #endif
  ...
  if (!surface)
  {
    #ifdef _DEBUG
      if (DEBUG==TRUE) 
        printf("platformRefreshCommitToSHM error at accesing surface\n");
    #endif
    // exit strategy
  }
  memcpy(
...
void *platformRefresh( struct platform_t *platform )
{
  ...
  #ifdef _DEBUG
    if (DEBUG==TRUE) printf("__interface platformRefresh\n");
  #endif
