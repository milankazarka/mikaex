...
#include "draqieConfigServerMod.h"
...
pthread_mutex_t draqieConfigServerMutex;
...
void *_process_data_config_server( unsigned char *in, unsigned char *out )
{
  ...
  if (run==0)
    pthread_mutex_init(&draqieConfigServerMutex,NULL);
  
  pthread_mutex_lock(&draqieConfigServerMutex);
  TiXmlDocument doc;
  doc.Parse(
    (const char*)in,
  ...
  if (root)
  {
    ...
    if (config)
    {
      if (strlen(config->GetText())+strlen(_ACTIVITIES_ROOT)+2<_ACTIVITY_PATH_LEN)
      {
        ...
        draqieConfigServerDirectoryListing((unsigned char*)activityPath,out);
      }
    }
  }
  
  run++;
  pthread_mutex_unlock(&draqieConfigServerMutex);
    
  return NULL;
}

int draqieConfigServerDirectoryListing( unsigned char *path, unsigned char *out )
{
  if (!path || !out)
    return 1;
  ...
  #ifdef _DEBUG
    printf("draqieConfigServerDirectoryListing path(%s)\n",(char*)path);
  #endif
  ...
  while((dp=readdir(dir)) != NULL)
  {
    ...
    if (strlen(dp->d_name)+strlen((char*)path)+2<_ACTIVITY_PATH_LEN+512)
    {
      ...
      #ifdef _DEBUG
        printf("draqieConfigServerDirectoryListing file(%s)\n",(char*)filename);
      #endif
    }
  }  
  closedir(dir);
