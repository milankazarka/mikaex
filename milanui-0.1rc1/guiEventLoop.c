    ...
    ievent.height = ieventptr->height;
        ...
        switch(ievent.type)
        {
          case INTERFACE_EVENT_BDOWN:
            ...
            #ifdef _DEBUG
              if (DEBUG==TRUE) 
                printf("gui_WidgetEventPush INTERFACE_EVENT_BDOWN\n");
            #endif 
          ...
          case INTERFACE_EVENT_BUP:
            ...
            else
            {
              ...
              {
                #ifdef _DEBUG
                  if (DEBUG==TRUE) printf(":stdonclick\n");
                #endif
              }
          ...
          */
          case INTERFACE_EVENT_MOTION:
            ...
            break; // wtf?
            if (main->attr.onmotion == NULL)
              break;
            e.widget = main;
            main->attr.onmotion(&e);
            main->attr.motion.x = e.x;
            main->attr.motion.y = e.y;
            break;
        
          case INTERFACE_EVENT_EXPOSE:
            
            #ifdef _DEBUG
              printf("gui_WidgetPushEvent\n");
            #endif
            ...
            #ifdef _DEBUG
              if (DEBUG==TRUE) printf(":Expose at %d %d %d %d\n",
                ievent.x, ievent.y,
                ievent.width,ievent.height);
            #endif
          
            #ifdef _EVENT_EXPOSE_SYNC
              trop_MemLock( );
            #endif
            ...
            while(current != NULL)
            {
              ...
              {
                #ifdef _DEBUG
                  if (DEBUG) printf("\t:widget.fall_through\n");
                #endif
                current = current->next;
                continue;
              }
            ...
            if (current->attr.onexposeEx)
            {
              ...
              if (current->attr.form == FORM_CANVAS)
              {
                ...
                {
                  #ifdef _DEBUG
                    if (DEBUG==TRUE) printf(":triggered\n");
                  #endif
                  break; // while(current
                }
              }
            ...
            {
              if (current->attr.form == FORM_NONE)
              {
                gui_WidgetExpose( current, current->attr.x, 
                  current->attr.x, current->attr.w, current->attr.h);
              }
              else if (current->attr.form == FORM_SLIDEBAR)
                form_SlidebarExpose( main, current );
              else if (current->attr.form == FORM_TITLEBAR)
                form_TitlebarExpose( current );
              else if (current->attr.form == FORM_FRAME)
                form_FrameExpose( current );
              else if (current->attr.form == FORM_BUTTON)
              {
                gui_WidgetExpose(
                  current,
                  current->attr.x,
              ...
              else if (current->attr.form == FORM_PROGRESS)
                form_ProgressExpose( current );
              else
              {
                // we don't support GUI objects directly in the main window,
                // only in sub widgets 
              }
            }
          ...
          if (expose!=NULL)
            expose = NULL;
          current = main;
          while(current != NULL) // overlay objects
          {
            if (current->attr.form == FORM_OVERLAY && 
              current->attr.bvisible == TRUE)
            {
              form_OverlayExpose( current );
            }
            current = current->next;
          }
          ...
          gui_GlColorSet(gl,&glpoly->color,140,140,140,140);
          ...
          if (REC_ENABLE == 1)
          {
            rec_ExposePrint(rec_expose);
            rec_ExposeToXML(rec_expose,(char*)str_rec_expose);
            rec_ExposeLogXML((char*)str_rec_expose,"rec_expose.xml");
            rec_ExposePlayXMLFile("rec_expose.xml");
          }
