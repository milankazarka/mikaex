...
void *_call_connection_processing_mods( struct mikaServer *server, struct mikaConnection *connection )
{
    ...
    int main( )
    {
        ...
        //struct mikaDb *db = mikaDbCreate(NULL,(unsigned char*)"db/test.db");
        ...
        strcpy((char*)modPos->id,"draqiePos");
        modPos->processData = _process_data_draqie_pos;
        
        configMod = mikaServerModInit();
        strcpy((char*)configMod->id,"draqieConfig");
        ...
        //server->callConnectionProcessing = _call_connection_processing_custom;
