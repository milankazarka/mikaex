...
struct gui_Widget *wCurrent = NULL;
  ...
  */
  ...
  {
    #ifdef _DEBUG
      printf("gui_Flush UEVENT EVENTFLOW\n");
    #endif
    return __x_flush();
  }
  ...
  */
  ...
  int _gui_surface_to_window( )
  {
    ...
    #else
      #ifdef _DEBUG
        printf("_gui_surface_to_window EVENTFLOW\n");
      #endif
  ...
  */
  ...
  int gui_DrawTextExA( struct gui_TextSim *ts, int x, int y, int w, int h, int round )
  {
    ...
    if (ts==NULL||w<1||h<1) return -1;
      ...
      #ifdef _DEBUG
        printf("gui_DrawTextExA\n");
      #endif
      if (!ts->str)
      {
        #ifdef _DEBUG
          printf("gui_DrawDebugExA error, no text\n");
        #endif
        return 1;
      }
      else
      {
        #ifdef _DEBUG
          printf("gui_DrawDebugExA text(%s)\n",ts->str);
        #endif
      }
      if (wCurrent)
      {
